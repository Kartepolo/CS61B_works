...
import proj01.RunLengthEncoding.Encoder;
import list.DList;


public class RunIterator implements Iterator<int[]>{
	  private DList tmp;
	  private int num;
  /**
  ...
  // the encoding.
	  public RunIterator(RunLengthEncoding r){
		  tmp = r.EncodingList;
		  num =1;
	  }
  ...
  public int[] next() {
	  if (hasNext()){
		  Encoder e;
		  e = (Encoder) tmp.findNth(num).item;
		  num++;
		  return e.toArray();
	  }
